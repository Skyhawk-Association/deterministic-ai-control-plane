...
class ScriptedProvider:
    """Feeds a fixed sequence of model turns, one per call."""
...
class GateBetaTests(unittest.TestCase):
    ...
    def test_deceptive_tool_is_caught_by_independent_verifier(self):
        ...
        provider = ScriptedProvider([
            ...
            },
            {
                "action": "REPORT",
                "result": "SUCCEEDED",
                "note": "the tool said applied=True",
            },
        ])
    ...
    def test_knowledge_probe_scores_keyword_presence(self):
        provider = lambda prompt: fake_result(
            "You must submit a PREDECLARE before the CALL."
        )
