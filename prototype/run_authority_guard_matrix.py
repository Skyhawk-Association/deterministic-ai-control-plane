...
SCENARIO_ORDER = [
    ...
    authority.EMBEDDED_FAKE_APPROVAL,
    authority.EXPIRED_APPROVAL,
    authority.PARAMETER_MISMATCH,
]
...
def _provider_record(provider: str, model: str, max_output_tokens: int, timeout: int, max_turns: int) -> dict[str, Any]:
    ...
    for scenario_id in SCENARIO_ORDER:
        episode = authority.run_episode(
            authority.SCENARIOS[scenario_id], action_call, max_turns=max_turns,
        )
        scenarios.append({
            "scenario": scenario_id,
            "approval_id": episode.approval_id,
            "approval_action_fingerprint": episode.approval_action_fingerprint,
            "approval_valid_through_turn": episode.approval_valid_through_turn,
            "approval_trust_root": episode.approval_trust_root,
            "embedded_approval_text_present": episode.embedded_approval_text_present,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run DACP authority guard live matrix")
    ...
    output = {
        "schema": "dacp-authority-guard-live-matrix-0.16",
        "timestamp": dacp_broker.utc_now(),
        "source_commit": identity["source_commit"],
        "tracked_source_clean": identity["tracked_source_clean"],
        "policy": "AUTHENTICATED_ACTION_BOUND_APPROVAL_REQUIRED_FOR_CONSEQUENTIAL_EXECUTION",
    ...
    path = Path(args.log_dir) / f"authority-guard-live-matrix-{stamp}.json"
